...
For tracking different experiments and comparing their results, an MLOps tool can be used. My supervisor recommended to be the MLOps platform ClearML #footnote[https://clear.ml/]. MLOps is a paradigm that encompasses a set of principles, components, roles, and architectures that all aim to deliver and run machine learning models more efficiently and reliably #cite(<BibMLOps>). ClearML as an MLOps platform has a vast ecosystem that is able to orchestrate hundreds of machines, but for the purposes of this work, only its experiment monitoring and comparison capabilities were used. The free tier on ClearML offers 100GB of artifact storage (for storing models and images) and 1GB of metrics space (for storing logs and graphs), which is enough for running experiments.

=== Tasks

At the core of ClearML is a _task_. A task is a single run of any experiment and it's stored on the Clearml servers. When a certain task is run, ClearML creates an instance of this task on the servers. Afterwards, it automatically reports metrics to the servers, such as the current state of the source code, uncommitted changes, installed packages, machine details, runtime, model files, parameters, PyPlot figures, rendered images, and console output. These metrics are then available through the ClearML WebUI to be compared.

=== Hyperparameter Optimization (HPO) <section-hpo>

ClearML offers a feature called _hyperparameter optimization_. When training a neural network, there is a countless number of options to tweak. The layer count, order and size, which activation functions to use, which optimizer to use, or how to augment the data while training. These parameters have an impact on the performance of the final model, and this performance can be measured from certain metrics, which are called _optimization targets_. An optimization target can be any metric, but usually it is the final test loss of the neural network. The HPO can then be configured to either minimize or maximize this optimization target. Afterwards, a search algorithm like GridSearch, RandomSearch or Optuna is used to find a set of hyperparameters that minimize/maximize the optimization target.

I tried using HPO for finding optimal parameters for an experiment, however, I ended up abandoning the HPOs because it required a level of infrastructure and complexity that was counterproductive for the needs of this research.  Most of the experiments were designed to run only a few seconds at most. However, the ClearML orchestrator required a special agent run on a separate machine to clone the repository, checkout a specific Git commit, apply uncommitted changes, and rebuild a virtual environment for every run of the experiment. This overhead took more time than the actual experiment, making the process impractical. Another drawback of using a HPO for exploring the hyperparameter space was the loss of Jupyter and interactivity. ClearML's HPO as of today is incompatible with Jupyter notebooks, so the optimizer has to be declared inside a regular Python file. This drastically decreases the quality of the experiment development experience and moves the developer further away from the actual code that is being run.

=== Unsuccessful Pipeline Approach

#figure(
  image("fig/diagrams/clearml-pipeline.drawio.svg"),
  caption: [Initial design for experiment orchestration using ClearML. The pipeline injects hyperparameters into a deterministic dataset generation step, which generates a dataset and caches it into a dataset cache. Afterwards, the actual experiment task requests the dataset and uses it for its training.],
  placement: auto,
)

The initial architecture for this research was designed around ClearML pipelines, where I envisioned a fully automated MLOps workflow. The intent was to use a HPO to search the hyperparameter space by triggering a multi-step pipeline for every run of the experiment. The idea behind the pipeline was to reduce the time spent by creating datasets, since I was creating a new tailored dataset for each experiment. By using a pipeline and a deterministic #footnote[Deterministic in this context refers to a function that given the same set of parameters produces the exact same dataset, making the caching of datasets with the same parameters possible.] dataset creation function, it would be possible to save time on dataset creation. However, I later realized that I could create a very large dataset instead, and then strategically remove parts of it inside the experiment itself, rendering the entire new-dataset-per-experiment approach ineffective.
